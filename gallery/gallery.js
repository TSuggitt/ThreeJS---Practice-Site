...
const frameTop = new THREE.Mesh(
  new THREE.BoxGeometry(6.2, frameWidth, frameDepth),
  frameMaterial
);
...
const slat1 = new THREE.Mesh(
  new THREE.BoxGeometry(6.2, frameDepth, frameDepth),
  frameMaterial
);
...
const wood_pattern = new THREE.TextureLoader().load(
  "img/beautiful-wood-texture-background.jpg"
);
...
function animateClouds() {
  clouds.forEach((cloud) => {
    ...
    tl3.fromTo(
      ...
      {
        x: cloud.position.x + Math.random() * 3,
        y: cloud.position.y + Math.random() * 2,
        z: cloud.position.z + Math.random() * 2,
      }
    );
  });
}
