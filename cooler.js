...
const floormaterial = new THREE.MeshStandardMaterial(
    {color: 0xffffff, side: THREE.DoubleSide, map:floor_pattern, metalness:0, roughness:0.5}
)
...
const cube_material = new THREE.MeshStandardMaterial({color:0x301934, emissive:0xff00ff, emissiveIntensity:1})
...
const paintmaterial1 = new THREE.MeshStandardMaterial({side:THREE.DoubleSide, map: mona_lisa})
...
const veteran = new THREE.TextureLoader().load("./img/veteraninnewfield-winslowhomer.jpeg")
const poppy = new THREE.TextureLoader().load("./img/monet-poppyfield.jpeg")

const painting2 = new THREE.PlaneGeometry(2.5,3)
const paintmaterial2 = new THREE.MeshStandardMaterial({side:THREE.DoubleSide, map: church})
...
const paintmaterial3 = new THREE.MeshStandardMaterial({side:THREE.DoubleSide, map: veteran})
...
const paintmaterial4 = new THREE.MeshStandardMaterial({side:THREE.DoubleSide, map:poppy})
...
const frame_material1 = new THREE.MeshStandardMaterial({ side:THREE.DoubleSide, metalness:1, roughness:0.2, color: 0xd4af37})
...
const frame_material2 = new THREE.MeshStandardMaterial({ side:THREE.DoubleSide, metalness:1, roughness:0.2, color: 0xd4af37})
...
const frame_material3 = new THREE.MeshStandardMaterial({ side:THREE.DoubleSide, metalness:1, roughness:0.2, color: 0xd4af37})
...
const frame_material4 = new THREE.MeshStandardMaterial({ side:THREE.DoubleSide, metalness:1, roughness:0.2, color: 0xd4af37})
...
// Number of bevel segments
    ...
    const tl4 = gsap.timeline({ defaults: { duration: 2.5, repeat: -1, yoyo: true }})
    const startRotation = { x: -0.1, y: -1, z: -0.1 }; 
    const endRotation = { x: 0.1, y: -0.58, z: 0.1}; 

    tl4.fromTo(textMesh.rotation, { x: startRotation.x, y: startRotation.y, z: startRotation.z }, { x: endRotation.x, y: endRotation.y, z: endRotation.z });
});
...
const tl3 = gsap.timeline({defaults:{duration:5, repeat:-1, yoyo:true}});
